...
import me.whiteship.springrestapidemo.common.BaseControllerTest;
...
public class EventControllerTests extends BaseControllerTest {
	...
	public void updateEvent() throws Exception {
		...
		this.mockMvc.perform(put("/api/events/{id}", event.getId())
				...
				.andExpect(jsonPath("name").exists())
				.andExpect(jsonPath("_links.self").exists())
				.andExpect(jsonPath("id", Matchers.equalTo(event.getId())))
				.andDo(document("update-event"));
	}

	@Test
	@TestDescription("입력값이 비어있는 경우에 이벤트 수정 실패")
	public void updateEvent_400_empty_parameter() throws Exception {
		// Given
		Event event = this.generateEvent(200);
	...
	@Test
	@TestDescription("입력값이 잘못된 경우에 이벤트 수정 실패, 데이터 바인딩 실패 (이름 없음)")
	public void updateEvent_400_wrong_parameter() throws Exception {
		// Given
		Event event = this.generateEvent(200);
		EventDto eventDto = this.modelMapper.map(event, EventDto.class);
